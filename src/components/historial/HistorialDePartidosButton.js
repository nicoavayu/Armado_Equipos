...
const HistorialDePartidosButton = ({ partidoFrecuente, className }) => {
  ...
  return (
    <>
      <button
        className={className || 'bg-white/[0.05] border border-[rgba(125,90,255,0.58)] rounded-none text-white/92 py-2.5 px-4 font-bebas text-base tracking-[0.01em] cursor-pointer transition-all duration-200 flex items-center justify-center flex-1 hover:bg-white/[0.1] hover:border-[rgba(125,90,255,0.7)] disabled:opacity-60 disabled:cursor-not-allowed disabled:transform-none sm:text-[13px] sm:px-3 sm:py-2 sm:min-h-[36px]'}
        onClick={handleClick}
        disabled={loading}
      >
