...
// Match-time universe is 07:00–00:00 (same rule as the manual wizard).
// '24:00' is the DB's end-of-day bound and is displayed as 00:00.
...
// Same select treatment as the "Nuevo partido" wizard time picker.
...
export default function AvailabilityOpportunityCard() {
  ...
  // Deep link from the Home carousel: /quiero-jugar?auto=1 opens the flow.
  useEffect(() => {
  ...
  const changeTimeStart = (value) => {
    setTimeStart(value);
    // Keep the range valid: the matcher needs at least a shared hour.
  ...
  const propose = async (format) => {
    ...
    try {
      ...
      setError(message.includes('active_proposal_exists')
        ? 'Ya participás de una propuesta activa.'
    ...
    <>
      {open ? (
        <div
          ...
          // z above the TabBar (z-[1000]): this is a full-screen flow, the tab
          // bar must not float over it.
        ...
        >
          ...
          <main className="relative z-10 mx-auto w-full max-w-[560px] px-4 pb-[max(34px,var(--safe-bottom,0px))] pt-[calc(var(--safe-top,0px)+92px)] font-oswald">
            <div className="mb-5 text-center">
              <p className="font-oswald text-[10px] font-semibold uppercase tracking-[0.22em] text-[#a98cff]">Buscar rivales y equipo</p>
              <h2 className="mt-1 font-bebas-real text-[clamp(36px,10vw,46px)] leading-[0.92] tracking-[0.035em] text-white drop-shadow-[0_8px_26px_rgba(5,2,20,0.7)]">
                QUIERO JUGAR
              </h2>
              <p className="mx-auto mt-2 max-w-[400px] font-oswald text-[12.5px] leading-relaxed text-white/52">
                Arma2 cruza tus días, horarios, formato y distancia con otros jugadores.
              </p>
            </div>
            ...
            {availability ? (
                      ...
                      <div key={item.format} className="rounded-2xl border border-white/[0.075] bg-black/15 p-3.5">
                        ...
                        {item.ready && !hasBlockingProposal ? (
                          <button
                            ...
                            className={`${PRIMARY_CTA_BUTTON_CLASS} mt-3 !min-h-[44px]`}
                          >
              ...
              <div className="rounded-card border border-[rgba(148,134,255,0.2)] bg-[linear-gradient(155deg,rgba(37,29,80,0.74),rgba(13,10,31,0.94))] p-4 shadow-[0_18px_46px_rgba(4,2,16,0.32),inset_0_1px_0_rgba(255,255,255,0.055)]">
                <p className="mb-4 font-oswald text-[12.5px] leading-relaxed text-white/56">
                  Elegí los días y el rango horario en los que podés jugar. La búsqueda queda activa hasta que la desactives.
                </p>
                ...
                <div className="mt-4 rounded-2xl border border-white/[0.075] bg-black/15 px-3.5 py-3">
                  <div className="mb-2 flex items-center gap-1.5 font-oswald text-[9px] font-semibold uppercase tracking-[0.14em] text-white/40">
                    <Clock3 size={13} className="text-[#9d82ff]" />
                    Rango horario
                  </div>
                    ...
                    <label>
                      ...
                      >
                        {endOptions.map((option) => (
                          <option key={option} value={option}>{displayTime(option)}</option>
                        ))}
                      </select>
                    </label>
                  </div>
