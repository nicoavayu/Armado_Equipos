...
-- Stage A interim relationship policy, the Stage A hotfix compatibility
-- policies (20260726120000) AND the legacy "Allow Insert Authenticated" policy
...
-- send_call_to_vote, enqueue_*), which generate content server-side and
...
-- Also drop the legacy CHECK(true) policy (finding #22) and every Stage A
-- hotfix compatibility policy so Stage B truly leaves ONLY self-insert.
-- Belt-and-suspenders: the 20260726120000 hotfix already dropped "Allow Insert
