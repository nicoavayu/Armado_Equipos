...
async function main() {
  ...
  await q('reset role');
  // Real domain state for the positive paths.
  await q("insert into public.player_awards (partido_id, jugador_id, award_type) values ($1,$2,'mvp')", [partidoId, players.target]);
  ...
  await expectError('award_won cannot be fabricated without a persisted award', 'authenticated', U.voterB,
  ...
  await expectOk('award_won is allowed against a persisted award', 'authenticated', U.creator,
