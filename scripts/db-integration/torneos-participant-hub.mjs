...
async function exercisePublishedVolume(admin, scope, official, participant) {
  await admin.query(
    `with generated as (
       select
         gen_random_uuid() id,
         series,
         format('Equipo QA %s', lpad(series::text, 3, '0')) name,
         format('equipo-qa-%s', lpad(series::text, 3, '0')) slug
       from generate_series(3, 100) series
     ), entries as (
       insert into public.tournament_team_entries(
         id,organization_id,season_id,tournament_id,category_id,name,slug,
         status,registration_source,created_by,submitted_by,submitted_at,
         reviewed_by,reviewed_at,approved_at,idempotency_key
       )
       select
         id,$1,$2,$3,$4,name,slug,'approved','manual',$5,$5,now(),
         $5,now(),now(),gen_random_uuid()
       from generated
       returning id,name
     )
     insert into public.tournament_competition_participants(
       ...
       team_entry_id,status,snapshot_name,frozen_at
     )
     select $1,$2,$3,$4,$6,id,'active',name,now()
     from entries`,
    [
      scope.organizationId,
      official.match.season_id,
      scope.tournamentId,
      scope.categoryId,
      USERS.owner,
      official.match.participant_set_id,
    ],
  );

  const rosterId = await value(
    admin,
    `select id from public.tournament_rosters
     where organization_id = $1 and team_entry_id = $2
       and status in ('approved','locked')
     order by version desc limit 1`,
    [scope.organizationId, scope.entries[0]],
  );
  const existingPlayers = Number(await value(
    admin,
    `select count(*)
     from public.tournament_roster_players player
     join public.tournament_rosters roster
       on roster.organization_id = player.organization_id
      and roster.team_entry_id = player.team_entry_id
      and roster.id = player.roster_id
      and roster.status in ('approved','locked')
     where player.organization_id = $1
  ...
  await admin.query(
    `with generated as (
       select
         gen_random_uuid() id,
         series,
         format('Jugador QA %s', lpad(series::text, 3, '0')) display_name
       from generate_series(1, $5::integer) series
     ), provisional as (
       insert into public.tournament_provisional_players(
         id,organization_id,display_name,normalized_name,created_by
       )
       select id,$1,display_name,lower(display_name),$4
       from generated
       returning id,display_name
     )
     insert into public.tournament_roster_players(
       organization_id,team_entry_id,roster_id,provisional_player_id,
       display_name,status,eligibility_status,added_by
     )
     select $1,$2,$3,id,display_name,'active','eligible',$4
     from provisional`,
    [
      scope.organizationId,
      scope.entries[0],
  ...
  await admin.query(
    `insert into public.tournament_matches(
     ...
     select
       organization_id,season_id,tournament_id,category_id,participant_set_id,
       fixture_version_id,phase_id,group_id,round_id,
       match_number + 1000 + series,leg_number,
       home_participant_id,away_participant_id,'unscheduled',created_by
     from public.tournament_matches source
     cross join generate_series(1, 499) series
     where source.id = $1`,
    [scope.matchId],
  );

  const startedAt = Date.now();
  const matchPage = await value(
    participant,
  ...
  for (const offset of [0, 32, 64, 96]) {
    teamPages.push(await value(
      participant,
      'select public.get_published_tournament_teams($1,$2,32,$3)',
      [scope.tournamentId, scope.categoryId, offset],
    ));
  }
  const elapsedMs = Date.now() - startedAt;
  const publishedPlayers = teamPages
    .flatMap((page) => page.items)
    .reduce((total, team) => total + team.roster.length, 0);

  eq(matchPage.pagination.total, 500, 'feed participante pagina un fixture de 500 partidos');
  eq(matchPage.items.length, 50, 'feed masivo conserva el límite máximo de 50 partidos');
  eq(teamPages[0].pagination.total, 100, 'directorio participante pagina 100 equipos');
  eq(publishedPlayers, 500, 'directorio masivo proyecta 500 jugadores sin N+1');
  ok(
    elapsedMs < 3000,
    'lectura local de 100 equipos, 500 jugadores y 500 partidos queda bajo 3 s',
    `${elapsedMs} ms`,
  );
}
...
async function exerciseHub(admin, scope, official) {
  ...
  await admin.query(
    `insert into public.tournament_match_availability_responses(
      organization_id,match_id,team_entry_id,roster_player_id,response,
      response_source,recorded_by,manual_reason
    ) values ($1,$2,$3,$4,'unavailable','manual',$5,'QA teammate privacy')`,
    [
      scope.organizationId,
      scope.matchId,
      scope.entries[0],
  ...
  await admin.query(
    `insert into public.tournament_player_suspensions(
      ...
      total_matches,status,reason
    ) values (
      $1,$2,$3,$4,$5,$6,$7,'direct_red','participant-hub-private',
      '{}'::jsonb,2,'active','Roja directa'
    )`,
    [
      official.revisionId,
      scope.organizationId,
      scope.tournamentId,
      scope.categoryId,
      official.match.phase_id,
      scope.rosterPlayers[0][1],
      scope.entries[0],
    ],
  );
  const draftSquadId = await value(
    admin,
  ...
  await admin.query(
    `insert into public.tournament_match_squad_players(
      ...
      position_snapshot,display_name_snapshot,is_goalkeeper,is_captain
    ) values (
      $1,$2,$3,$4,$5,'available','called_up','starter',1,'ARQ',
      'Jugador Napoli',true,false
    )`,
    [
      scope.organizationId,
      draftSquadId,
  ...
  await admin.query(
    ...
    [scope.organizationId, USERS.playerHome, USERS.owner],
  );
  const composedList = await value(
    playerHome,
    'select public.get_my_tournament_memberships(20,0)',
  );
  eq(
    composedList.items.length,
    1,
    'Mis torneos deduplica jugador y miembro organizacional en la misma categoría',
  );
  eq(
    composedList.items[0].organizationRole,
    'collaborator',
    'Mis torneos conserva los roles compuestos sin duplicar la tarjeta',
  );
  await admin.query(
  ...
  eq(
    hub.myTeam.activeSuspensions.length,
    0,
    'jugador no recibe sanciones privadas de compañeros',
  );
  eq(
    hub.myTeam.nextMatchResponses.total,
    0,
    'jugador no recibe disponibilidad agregada del plantel',
  );
  ok(
  ...
  eq(
    matches.items[0].myCallupStatus,
    null,
    'convocatoria draft no se filtra al jugador',
  );
  ok(
  ...
  eq(
    detail.myContext.callup,
    null,
    'detalle no expone una convocatoria todavía en draft',
  );
  ok(
  ...
  const captainHub = await value(
    captainHome,
    'select public.get_tournament_participant_hub($1,$2)',
    [scope.tournamentId, scope.categoryId],
  );
  eq(
    captainHub.myTeam.activeSuspensions.length,
    1,
    'capitán ve sanciones publicadas relevantes para convocar',
  );
  eq(
    captainHub.myTeam.nextMatchResponses.total,
    1,
    'capitán ve respuestas del plantel propio',
  );
  await admin.query(
    `update public.tournament_match_squads
     set status = 'submitted', submitted_by = $2, submitted_at = now()
     where id = $1`,
    [draftSquadId, USERS.captainHome],
  );
  const publishedCallup = await value(
    playerHome,
    `select public.get_published_tournament_matches(
      $1,$2,'results',null,10,0
    )`,
  ...
  eq(
    publishedCallup.items[0].myCallupStatus,
    'called_up',
    'convocatoria propia aparece después de presentarse',
  );
  ...
  await admin.query(
    `update public.tournament_team_managers
     set role = 'assistant'
     where organization_id = $1 and user_id = $2`,
    [scope.organizationId, USERS.captainHome],
  );
  await expectError(
    () => captainHome.query(
      'select public.get_tournament_participant_hub($1,$2)',
    ...
    /TORNEOS_HUB_FORBIDDEN/,
    'assistant activo no obtiene acceso reservado a capitán o delegado',
  );
  await admin.query(
    `update public.tournament_team_managers
     set role = 'captain'
  ...
  await expectError(
    ...
    /TORNEOS_HUB_FORBIDDEN/,
    'capitán revocado pierde acceso inmediatamente',
  );
  await admin.query(
    `update public.tournament_team_managers
  ...
  const claimedProvisionalId = await value(
    admin,
    `select provisional_player_id
     from public.tournament_roster_players
     where id = $1`,
    [scope.rosterPlayers[0][1]],
  );
  await admin.query(
    `update public.tournament_provisional_players
     set claim_status = 'claimed', claimed_by_user_id = $2
     where id = $1`,
    [claimedProvisionalId, USERS.outsider],
  );
  const linkedProvisionalHub = await value(
    outsider,
    'select public.get_tournament_participant_hub($1,$2)',
    [scope.tournamentId, scope.categoryId],
  );
  eq(
    linkedProvisionalHub.audience.isPlayer,
    true,
    'jugador provisional vinculado obtiene su relación actual',
  );
  await admin.query(
    `update public.tournament_provisional_players
     set claim_status = 'unclaimed', claimed_by_user_id = null
     where id = $1`,
    [claimedProvisionalId],
  );

  const historicalRosterId = await value(
    admin,
    `insert into public.tournament_rosters(
      organization_id,team_entry_id,version,status,submitted_at,approved_at,
      created_by
    ) values ($1,$2,2,'superseded',now(),now(),$3) returning id`,
    [scope.organizationId, scope.entries[0], USERS.owner],
  );
  await admin.query(
    `insert into public.tournament_roster_players(
      organization_id,team_entry_id,roster_id,arma2_user_id,display_name,
      shirt_number,primary_position,is_goalkeeper,status,eligibility_status,
      added_by
    ) values ($1,$2,$3,$4,'Jugador histórico',99,'DEF',false,'active','eligible',$5)`,
    [
      scope.organizationId,
      scope.entries[0],
      historicalRosterId,
  ...
  await expectError(
    ...
    /TORNEOS_HUB_FORBIDDEN/,
    'un plantel superseded no conserva acceso privado',
  );
  ...
  await expectError(
    ...
    /TORNEOS_HUB_FORBIDDEN/,
    'equipo retirado deja de autorizar al jugador',
  );
  await expectError(
    () => captainHome.query(
      'select public.get_tournament_participant_hub($1,$2)',
    ...
    /TORNEOS_HUB_FORBIDDEN/,
    'equipo retirado deja de autorizar al capitán',
  );
  await admin.query(
    `update public.tournament_team_entries
  ...
  const publishedStandingsOnly = await value(
    scope.owner,
    `select public.get_published_tournament_standings(
      $1,$2,$3,null
    )`,
    [scope.tournamentId, scope.categoryId, official.match.phase_id],
  );
  eq(
    publishedStandingsOnly.standings.length,
    0,
    'RPC participante de tabla no devuelve drafts ni a un owner',
  );
  const publishedStatisticsOnly = await value(
    scope.owner,
    `select public.get_published_tournament_statistics(
      $1,$2,$3,null
    )`,
    [scope.tournamentId, scope.categoryId, official.match.phase_id],
  );
  eq(
    publishedStatisticsOnly.players.length,
    0,
    'RPC participante de estadísticas no devuelve drafts ni a un owner',
  );
  await admin.query(
    `update public.tournament_standings_revisions
  ...
  const safeStatistics = await value(
    scope.owner,
    `select public.get_published_tournament_statistics(
      $1,$2,$3,null
    )`,
  ...
  eq(safeStatistics.players.length, 2, 'estadísticas publicadas vuelven al endpoint participante');
  ok(
    !JSON.stringify(safeStatistics).match(/avatar|fingerprint|rebuild|reason|audit/i),
    'estadísticas participantes omiten avatares y metadatos administrativos',
  );

  await exercisePublishedVolume(admin, scope, official, playerHome);
  ...
  eq(
    archivedHub.myTeam.activeSuspensions.length,
    0,
    'hub histórico no conserva sanciones privadas del plantel',
  );
  eq(
    archivedHub.myTeam.nextMatchResponses.total,
    0,
    'hub histórico no conserva disponibilidad privada',
  );
  const archivedMatches = await value(
    captainHome,
    `select public.get_published_tournament_matches(
      $1,$2,'results',null,10,0
    )`,
  ...
  eq(
    archivedMatches.items[0].myCallupStatus,
    null,
    'archivo histórico oculta convocatorias privadas',
  );
  const archivedDetail = await value(
    playerHome,
    'select public.get_tournament_participant_match($1)',
    [scope.matchId],
  );
  eq(
    archivedDetail.myContext,
    null,
    'detalle histórico no conserva disponibilidad ni convocatoria privadas',
  );
  await admin.query(
  ...
  await expectError(
    ...
    /TORNEOS_HUB_FORBIDDEN/,
    'categoría archivada deja de autorizar el hub',
  );
  await admin.query(
    `update public.tournament_categories
  ...
  await expectError(
    ...
    /TORNEOS_HUB_FORBIDDEN/,
    'organización archivada invalida todas las relaciones',
  );
}
  ...
  try {
    ...
    const publicExecute = Number(await value(
      ...
      [[
        ...
        'get_my_current_tournament_roster_players',
    ...
    eq(helperExecute, 0, 'helpers de autorización no son ejecutables por cliente');
    ...
    eq(securityDefiners, 10, 'RPCs fijan search_path y usan autoridad backend');
    const indexes = Number(await value(
      ...
      [[
        ...
        'tournament_provisional_players_claimed_user_idx',
        'tournament_roster_players_provisional_active_idx',
        'tournament_matches_participant_feed_idx',
      ]],
    ));
    eq(indexes, 4, 'consultas reales del hub tienen índices dedicados');
