...
-- Allow cancelling a challenge-origin team_match even if the linked challenge
-- was already moved to canceled/cancelled by another flow.
...
DECLARE
  ...
  v_challenge_is_canceled boolean;
BEGIN
  ...
  IF NEW.challenge_id IS NOT NULL THEN
    ...
    v_challenge_is_canceled := lower(COALESCE(v_challenge.status, '')) IN ('canceled', 'cancelled');

    IF v_challenge.status NOT IN ('accepted', 'confirmed', 'completed')
       AND NOT (v_is_cancel_transition AND v_challenge_is_canceled) THEN
