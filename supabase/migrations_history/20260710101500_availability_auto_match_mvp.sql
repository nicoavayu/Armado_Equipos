-- Availability -> matches -> automatic match proposal MVP (weekly model)
-- Users declare recurring availability: days of the week + one time range,
-- not concrete dates. The search stays active until the user turns it off.
-- The MVP deliberately creates proposals, not live matches: every matched
-- player must opt in before an administrator turns the proposal into a
-- normal partido.

-- Nothing of the v1 (date-window) draft of this feature ever reached prod;
-- these guarded drops only clean up environments that applied that draft.
drop function if exists public.find_my_availability_matches(integer);
drop function if exists public.upsert_my_availability(timestamptz,timestamptz,text[],integer,double precision,double precision);
drop table if exists public.auto_match_proposal_members cascade;
drop table if exists public.auto_match_proposals cascade;
drop table if exists public.player_availability cascade;
...
create table public.player_availability (
  ...
  -- Local-time range shared by every selected day. time_end may be '24:00'
  -- (Postgres accepts it) so a window can run through the end of the day.
  ...
  constraint player_availability_days_check check (
    cardinality(days_of_week) between 1 and 7
    and days_of_week <@ array[1,2,3,4,5,6,7]::smallint[]
  ),
  constraint player_availability_time_check check (
    time_end > time_start
    and time_end - time_start >= interval '60 minutes'
  ),
...
create unique index player_availability_one_active_per_user
  on public.player_availability(user_id)
  where status = 'active';
create index player_availability_active_idx
  on public.player_availability(status);

-- Bitmask of ISO weekdays (bit 1<<d per day) so group day-intersections can
-- be computed with plain integer AND.
create or replace function public.availability_days_mask(p_days smallint[])
returns integer
language sql
immutable
as $$
  select coalesce(sum(distinct 1 << d::integer)::integer, 0)
  from unnest(p_days) as d
  where d between 1 and 7;
$$;

create table public.auto_match_proposals (
  id bigint generated by default as identity primary key,
...
create table public.auto_match_proposal_members (
...
begin
  ...
  if p_days is null or cardinality(p_days) = 0 or not (p_days <@ array[1,2,3,4,5,6,7]::smallint[]) then
    raise exception 'invalid_days';
  end if;
  if p_time_start is null or p_time_end is null or p_time_end <= p_time_start then
...
as $$
  ...
  select
    ...
    array(
      ...
      intersect
      select unnest(other.days_of_week)
      order by 1
    ) as shared_days,
    greatest(mine.time_start, other.time_start) as window_start,
    least(mine.time_end, other.time_end) as window_end,
    ...
    floor(extract(epoch from (least(mine.time_end, other.time_end) - greatest(mine.time_start, other.time_start))) / 60)::integer as overlap_minutes
  from mine
...
revoke execute on function public.availability_days_mask(smallint[]) from public, anon;
revoke execute on function public.upsert_my_availability(smallint[],time,time,text[],integer,double precision,double precision) from public, anon;
