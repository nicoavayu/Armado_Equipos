...
import {
  TEAM_PHOTO_ALLOWED_MIME,
  TEAM_PHOTO_MAX_EDGE,
  TEAM_PHOTO_MAX_FILE_BYTES,
  TEAM_PHOTO_MAX_PIXELS,
  TEAM_PHOTO_PATH_RE,
  TEAM_PHOTO_SIGNED_URL_TTL_SECONDS,
  TOURNAMENT_TEAM_PHOTOS_BUCKET,
} from "../_shared/tournamentTeamPhotoContract.ts"

type ServiceClient = ReturnType<typeof createClient>

function mapError(message: string) {
  if (message.includes("TORNEOS_TEAM_PHOTO_FILE_INVALID")) return [422, "file_invalid"] as const
  if (message.includes("TORNEOS_TEAM_PHOTO_CHECKSUM_INVALID")) return [422, "checksum_invalid"] as const
  if (message.includes("TORNEOS_TEAM_PHOTO_STATE_INVALID")) return [422, "state_invalid"] as const
  if (message.includes("TORNEOS_TEAM_PHOTO_AUDIENCE_DISABLED")) return [403, "audience_disabled"] as const
  if (message.includes("TORNEOS_TEAM_PHOTO_FORBIDDEN")) return [403, "forbidden"] as const
  if (message.includes("TORNEOS_AUTH_REQUIRED")) return [401, "auth_required"] as const
  return [500, "team_photo_service_failed"] as const
}
...
function validGrant(grant: Record<string, unknown>) {
  return grant.bucket === TOURNAMENT_TEAM_PHOTOS_BUCKET
    && TEAM_PHOTO_PATH_RE.test(String(grant.objectPath ?? ""))
}

/**
 * El checksum lo calcula este servicio sobre los bytes que efectivamente subió,
 * nunca el navegador: un hash declarado por quien manda el archivo no prueba
 * nada sobre lo que quedó guardado.
 */
...
async function upload(
  ...
  const teamEntryId = url.searchParams.get("teamEntryId") ?? ""
  ...
  if (!UUID_RE.test(organizationId) || !UUID_RE.test(teamEntryId)
    || !(TEAM_PHOTO_ALLOWED_MIME as readonly string[]).includes(mimeType)
    || !Number.isSafeInteger(declaredBytes) || declaredBytes < 1
    || declaredBytes > TEAM_PHOTO_MAX_FILE_BYTES
    || !Number.isSafeInteger(width) || width < 1 || width > TEAM_PHOTO_MAX_EDGE
    || !Number.isSafeInteger(height) || height < 1 || height > TEAM_PHOTO_MAX_EDGE
    || width * height > TEAM_PHOTO_MAX_PIXELS) {
    return { status: 422, payload: { error: "file_invalid" } }
  }
  const bytes = new Uint8Array(await request.arrayBuffer())
  if (bytes.length !== declaredBytes || bytes.length > TEAM_PHOTO_MAX_FILE_BYTES) {
    return { status: 422, payload: { error: "file_invalid" } }
  }
  const requested = await service.rpc("request_tournament_team_photo_upload", {
  ...
  const teamPhotoId = String(grant.teamPhotoId)
  const objectPath = String(grant.objectPath)
  const stored = await service.storage.from(TOURNAMENT_TEAM_PHOTOS_BUCKET)
    .upload(objectPath, bytes, { contentType: mimeType, cacheControl: "0", upsert: false })
  if (stored.error) {
    await service.rpc("fail_tournament_team_photo_upload", {
      p_actor_user_id: actorId, p_team_photo_id: teamPhotoId,
    })
    return { status: 502, payload: { error: "storage_unavailable" } }
  }
  const finalized = await service.rpc("finalize_tournament_team_photo_upload", {
    p_actor_user_id: actorId,
    p_team_photo_id: teamPhotoId,
    p_checksum_sha256: await sha256Hex(bytes),
  })
  if (finalized.error || !finalized.data) {
    await service.storage.from(TOURNAMENT_TEAM_PHOTOS_BUCKET).remove([objectPath])
    await service.rpc("fail_tournament_team_photo_upload", {
      p_actor_user_id: actorId, p_team_photo_id: teamPhotoId,
    })
...
async function resolve(
  ...
  if (ref?.kind !== "team_photo" || !UUID_RE.test(id)) {
    return { status: 400, payload: { error: "invalid_image_ref" } }
  }
  const authorized = await service.rpc("authorize_tournament_team_photo_read", {
    p_actor_user_id: actorId, p_team_photo_id: id,
  ...
  const signed = await service.storage.from(TOURNAMENT_TEAM_PHOTOS_BUCKET)
    .createSignedUrl(String(grant.objectPath), TEAM_PHOTO_SIGNED_URL_TTL_SECONDS)
  ...
  return {
    ...
    payload: {
      ...
      ttlSeconds: TEAM_PHOTO_SIGNED_URL_TTL_SECONDS,
...
async function remove(
  ...
  const teamPhotoId = String(body.teamPhotoId ?? "")
  if (!UUID_RE.test(teamPhotoId)) {
    return { status: 400, payload: { error: "invalid_request" } }
  }
  const begun = await service.rpc("begin_tournament_team_photo_delete", {
    p_actor_user_id: actorId, p_team_photo_id: teamPhotoId,
  })
  ...
  const deleted = await service.storage.from(TOURNAMENT_TEAM_PHOTOS_BUCKET)
    .remove([String(grant.objectPath)])
  if (deleted.error) {
    return { status: 502, payload: { error: "delete_storage_failed", deletePending: true } }
  }
  const completed = await service.rpc("complete_tournament_team_photo_delete", {
    p_actor_user_id: actorId, p_team_photo_id: teamPhotoId,
  })
...
serve(async (request) => {
  ...
  try {
    ...
    return jsonResponse(cors, 500, { error: "team_photo_service_failed" })
  }
})
